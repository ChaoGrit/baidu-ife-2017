...
var
	...
	/*chao: $()和jQuery()都是调用这个函数，返回对象,才能在之后调用方法*/
	...
	// Match a standalone tag匹配单标签-> 
	rsingleTag = /^<(\w+)\s*\/?>(?:<\/\1>|)$/,/*chao:<li>或<li></li>可以,<li></li><li></li>不行*/
	...
	fcamelCase = function( all, letter ) {//两个参数：正则，整体
...
jQuery.fn = jQuery.prototype = {//定义了一些jQuery的实例方法，放在prototype上
	...
	pushStack: function( elems ) {//elems是被选择的jq对象

		// Build a new jQuery matched element set
		var ret = jQuery.merge( this.constructor(), elems );//空jQ对象整合elems，返回对应的格式

		// Add the old object onto the stack (as a reference)
		ret.prevObject = this;//寄存前一层的栈，用于end
	...
	//工具方法(最底层)->实例方法(高一层，给jquery对象用的)
	each: function( callback, args ) {//实例方法
		return jQuery.each( this, callback, args );//调用jquery工具方法中的each
	},

	ready: function( fn ) {
		// Add the callback
		jQuery.ready.promise().done( fn );//调用内部函数，延迟对象
	...
	eq: function( i ) {
		var len = this.length,
			j = +i + ( i < 0 ? len : 0 );//对i是正负进行选择，产生正向顺序
		return this.pushStack( j >= 0 && j < len ? [ this[j] ] : [] );
	},

	map: function( callback ) {//对数组进行二次处理，return返回处理之后的方法
		return this.pushStack( jQuery.map(this, function( elem, i ) {//调用工具方法
			return callback.call( elem, i, elem );
		}));
	},

	end: function() {//找到栈的下一层
		return this.prevObject || this.constructor(null);//返回上一级对象，或空jq对象
	},

	// For internal use only.
	// Behaves like an Array's method, not like a jQuery method.
	//把数组的方法挂在jquery方法上，不建议在外部使用
	push: core_push,
	sort: [].sort,
	splice: [].splice
};
